 ...
 * 抖音统一上传脚本
 * 用法:
 *   视频: npx tsx scripts/upload.ts --type video --video <path> --title <title> [options]
 *   图文: npx tsx scripts/upload.ts --type image --images <path1,path2,...> --description <text> [options]
 */
...
function printUsage() {
  console.log(`
Usage:
  Video: npx tsx scripts/upload.ts --type video --video <path> --title <title> [options]
  Image: npx tsx scripts/upload.ts --type image --images <path1,path2,...> --description <text> [options]
...
Examples:
  npx tsx scripts/upload.ts --type video --video "./video.mp4" --title "My Video" --tags "fun,daily"
  npx tsx scripts/upload.ts --type image -i "./a.jpg,./b.jpg" -d "今日随拍" --tags "日常,摄影"
`);
}
...
async function main() {
  ...
  if (args.type === 'video') {
    ...
    if (!args.title) {
      console.error('❌ Error: --title is required when --type is video\n');
      printUsage();
      process.exit(1);
    }
